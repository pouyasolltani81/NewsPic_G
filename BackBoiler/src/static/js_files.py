MESSAGES_JS = r"""// Global queue for bulk messages
let messageQueue = [];
let totalMessages = 0;
let sentMessages = 0;
    ...
    document.getElementById("modal-yes").onclick = async () => {
      ...
      if (sendToAll) {
        try {
          ...
          const recipientIds = data.user_ids;

          // Clear any existing queue and reset counters
          messageQueue = [];
          totalMessages = recipientIds.length;
          sentMessages = 0;



          // Create individual FormData for each recipient
          recipientIds.forEach(userId => {
            const individualFd = new FormData();

            // Copy all fields from original form
            for (let [key, value] of fd.entries()) {
              individualFd.append(key, value);
            }

            individualFd.delete('to_user');

            individualFd.append('to_user', userId);



            individualFd.append('to_user_id', userId);


            // Debug: Log all user IDs received
            console.log('[Bulk Send] Received user IDs from server:', recipientIds);
            console.log('[Bulk Send] Total users to send to:', recipientIds.length);
            console.log('[Bulk Send] Current user ID:', document.body.dataset.userId);

            // Add to queue
            messageQueue.push(individualFd);
          });
...
function getNextQueuedMessage() {
  if (messageQueue.length > 0) {
    ...
    console.log(`[Bulk Send] Message sent successfully to user ID: ${userId} (${sentMessages}/${totalMessages})`);
...
QUICKACTIONS_JS = r"""const quickActions = [
    {
